    ...
    try {
      ...
      if (value.startsWith('***')) {
        ...
        if (num1.isEmpty) {
          ...
          final index = rooms.lastIndexOf(num2);
  ...
  void PlaySound(String num1, String num2) async {
    try {
      ...
      Future<void> playNumberSound(String number) async {
        try {
          for (int i = 0; i < number.length; i++) {
            await player
                .play(AssetSource('sounds/${number[i]}.mp3'))
                .catchError((e) {
              _showErrorDialog(e.toString());
            });
      ...
      if (mode == '1') {
        ...
        await playSoundtrack('soundtrack/ที่ห้องตรวจหมายเลข.mp3', 1500);
        await playNumberSound(num2);
      } else if (mode == '2') {
        await playSoundtrack('soundtrack/เชิญหมายเลข.mp3', 1200);
        await playNumberSound(num1);
        await playSoundtrack('soundtrack/ที่เค้าเตอร์หมายเลข.mp3', 1500);
        await playNumberSound(num2);
      } else if (mode == '3') {
        await playSoundtrack('soundtrack/เชิญหมายเลข.mp3', 1200);
        await playNumberSound(num1);
        await playSoundtrack('soundtrack/ที่ช่องบริการ.mp3', 1500);
        await playNumberSound(num2);
      }
