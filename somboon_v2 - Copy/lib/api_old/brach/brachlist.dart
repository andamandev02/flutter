...
class ClassBranch {
  ...
  static Future<void> reasondetail({
    required BuildContext context,
    required Function(List<Map<String, dynamic>>) onReasonDetailLoaded,
  }) async {
    try {
      final uri = Uri.parse(
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData.containsKey('data') && jsonData['data'] is List) {
          List<Map<String, dynamic>> Reason = (jsonData['data'] as List)
              .map((item) => item as Map<String, dynamic>)
              .toList();
          onReasonDetailLoaded(Reason);
        } else {
          onReasonDetailLoaded([]);
        }
      } else {
        onReasonDetailLoaded([]);
      }
  ...
  static Future<void> EndQueueReasonlist({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print('Finish:  to load data. Status code: ${response.statusCode}');
        Map<String, dynamic> jsonData = jsonDecode(response.body);
        List<Map<String, dynamic>> Reason = (jsonData['data'] as List)
            .map((item) => item as Map<String, dynamic>)
            .toList();
